...
void BiConvolution3DLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  // For Binary
  this->W_b.reset(new Blob<Dtype>());
  this->W_buffer.reset(new Blob<Dtype>());
...
// For Binary
template <typename Dtype>
void BiConvolution3DLayer<Dtype>::binarizeCPUTo(const shared_ptr<Blob<Dtype> > weights, const shared_ptr<Blob<Dtype> > wb) {
  ...
  for (int num = 0; num < weights->num(); num++) {
    ...
    for (int _l = 0; _l < weights->length(); _l++)
    for (int _h = 0; _h < weights->height(); _h++)
    for (int _w = 0; _w < weights->width(); _w++)
      A[num] += std::abs(weights->data_at(num, _c, _l, _h, _w)) / Dtype(div);
  }
...
template <typename Dtype>
Dtype BiConvolution3DLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
...
template <typename Dtype>
void BiConvolution3DLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_CLASS(BiConvolution3DLayer);
